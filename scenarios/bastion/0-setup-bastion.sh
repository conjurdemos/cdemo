...
load_policy() {
	...
	docker exec $CLI_CONT_NAME conjur authn login -u admin -p $CONJUR_MASTER_PASSWORD
	docker exec $CLI_CONT_NAME conjur policy load --as-group=security_admin /src/scenarios/bastion/$ACCESS_POLICY_FILE
}
...
bring_up_vms() {
	...
	docker-compose -f ../../docker-compose.yml rm -svf $BASTION_SERVICES
	docker-compose -f ../../docker-compose.yml up -d $BASTION_SERVICES
}
...
conjurize_vms() {
	...
	api_key=$(docker exec $CLI_CONT_NAME conjur host rotate_api_key --host bastion/server)
	conjurize_container_as_host bastion_server bastion/server $api_key

	hf_token=$(docker exec $CLI_CONT_NAME conjur hostfactory tokens create --duration-minutes=1 protected | jq -r .[].token) 
        api_key=$(docker exec $CLI_CONT_NAME conjur hostfactory hosts create $hf_token protected/vm | jq -r .api_key)
...
setup_user() {
	...
	docker-compose exec -T cli conjur pubkeys add $user @/src/scenarios/bastion/id_$user.pub

  	docker exec outside_vm sudo useradd -m $user
	docker exec outside_vm sudo su $user -c 'mkdir ~/.ssh'
	docker exec outside_vm sudo cp /src/scenarios/bastion/id_$user /home/$user/.ssh/id_rsa

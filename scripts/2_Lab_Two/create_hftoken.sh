#!/bin/bash
#
# Admin_process - simulates the role of a security administrator that creates and distributes Host Factory 
tokens
#
...
urlify() {
        ...
        str=$(echo $str | sed 's=+=-=g')   # added as hack to change + to - in timezone offset in linux date 
string
        URLIFIED=$str
}
